;;; -*- Mode: Common-LISP -*-
;;;
;;;RESOLU.LSP    0.4 18-Mar-83 0143 000   1 ML E      18-Mar-83   
;;;	Resolution functions
;;;
;;;perm filename RESOLU.LSP[MRS,LSP] blob sn#702134 filedate
...
;;; (declare (special alist theory truth agenda))



(defun resolution (x)
  (let ((theory (newtheory theory)) (gl) (vl) (ans))
    (setq gl (cnf (maknot x)) vl (variables x truth))
    (do ((l gl (cdr l)))
	((null l))
      (if (null (cdar l)) (stash (caar l))
	  (mrs-assert-or (cons 'or (car l)))))
    (do ((l gl (cdr l)))
	((null l))
...
(defun newtheory (d) (let ((c (maksym 't))) (includes c d) c))
...
(defun rslookup (gl al)
  (do ((l (lookups (maknot (car gl))) (cdr l)))
      ((null l)) 
...
(defun resolutionresidue (x)
  (let ((theory (newtheory theory)) (gl) (ans))
